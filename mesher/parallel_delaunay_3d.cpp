...
ParallelDelaunayTree3D::ParallelDelaunayTree3D(Point * p0,  Point *p1,  Point *p2,  Point *p3, const std::vector<const Geometry *> & domains) : Mesh< DelaunayTetrahedron, DelaunayTreeItem3D>(SPACE_THREE_DIMENSIONAL), domains(domains)
{
...
Vector ParallelDelaunayTree3D::getSmoothedField ( FieldType f0, int cacheID, IntegrableEntity* e,  double t ,const std::vector<bool> & restrict, int index ) {
    ...
    VirtualMachine vm ;
    if ( f0 == PRINCIPAL_MECHANICAL_STRAIN_FIELD || f0 == REAL_STRESS_FIELD || f0 == EFFECTIVE_STRESS_FIELD || f0 == PRINCIPAL_REAL_STRESS_FIELD || f0 == PRINCIPAL_EFFECTIVE_STRESS_FIELD ) {
