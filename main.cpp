...
setPointAlong(XI, x, point.id)
setPointAlong(ETA, y, point.id)
setPointAlong(ZETA, z, point.id)
...
// 			}
// 			
//		  if (std::abs(triangles[k]->getBoundingPoint(c).y ) == 0.02)
		  if ( triangles[k]->getBoundingPoint(c).y  < -0.01999)
			{
			...
			if (  triangles[k]->getBoundingPoint(c).y >  0.01999 )
			{
				featureTree->getAssembly()->setPoint( 0, .000001 ,triangles[k]->getBoundingPoint(c).id) ;
 			}
...
/*		bool in = !triangles[k]->getEnrichmentFunctions().empty() ;*/
		...
		double delta_r = sqrt(aggregateArea*0.03/((double)zones.size()*M_PI))/60. ;
	...
	for(size_t i = 0 ; i < expansion_reaction.size() ; i++)
		std::cout << expansion_reaction[i].first << "   " << expansion_reaction[i].second << std::endl ;
}
...
int main(int argc, char *argv[])
{
	...
	// parent, length, height, centrex, centrey
...
// 	reinforcement1.setBehaviour(new Stiffness(m0*5)) ;
	...
	Inclusion * inc = new Inclusion(.004, -0.01,0.000) ;
	std::vector<Inclusion *> inclusions ;
// 	inclusions = GranuloBolome(.15, 25000, BOLOME_A)(.005, .999);
...
// 	sample.setBehaviour(new BimaterialInterface(&cercle, m0,  m0*4)) ;
	...
	ptset1[0] = new Point(x_0, y_0) ;
	ptset1[1] = new Point(x_1, y_1) ;
	crack.push_back(new Crack(ptset1, 0.02)) ;
	...
	for(size_t j = 0 ; j < crack.size() ; j++)
		crack[j]->setInfluenceRadius(0.003) ;
// 	
	step() ;
