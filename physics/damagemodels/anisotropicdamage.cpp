...
// 	double maxincrement = std::abs((l_2*E_2-1.)/(l_2+l_2*E_2)) ;
    ...
    else
    {
        ...
        s.getAverageField( REAL_STRESS_FIELD, stress, nullptr, 0) ;
...
Matrix AnisotropicLinearDamage::apply(const Matrix & m, const Point & p, const IntegrableEntity * e, int g ) const
{
    ...
    ret[0][0] = m[0][0]*(1.-rstate[0]) ;
    ret[0][1] = m[0][1]*sqrt((1.-rstate[0])*(1.-rstate[1])) ;
    ret[0][2] = 0 ;
    ret[1][0] = m[1][0]*sqrt((1.-rstate[0])*(1.-rstate[1]))  ;
    ret[1][1] = m[1][1]*(1.-rstate[1])  ;
    ret[1][2] = 0 ;
    ret[2][0] = 0 ;
    ret[2][1] = 0 ;
    ret[2][2] = m[2][2]*sqrt((1.-rstate[0])*(1.-rstate[1])) ;
    return ret ;
