...
public:
    ...
    double getDamageDensityTolerance() {
        return damageDensityTolerance ;
    };
     ...
     * @return a Vector
     */
    virtual const Vector & getState() const final {
        return state ;
    } ;
    ...
    virtual Matrix applyViscous(const Matrix & m, const Point & p = Point(), const IntegrableEntity * e = nullptr, int g = -1) {
        return m ;
    }
...
public:
     ...
     * @param s ElementState
     */
    virtual std::pair<Vector, Vector> computeDamageIncrement(ElementState & s)  {
        return std::make_pair(Vector(1), Vector(1)) ;
    } /*override*/;
    ...
    /** \brief returns 0 */
    virtual Vector getPreviousPreviousDamage() {
        return Vector(0) ;
    } ;
