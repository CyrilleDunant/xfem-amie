...
Vector GeneralizedIterativeMaxwell::getImposedStress(const Point & p, IntegrableEntity * e, int g) const
{
    ...
    if(e)
    {
        if(imposedStressAtGaussPoints.size() > 1)
        {
            ...
            VirtualMachine vm ;
            Vector ret ;
            ret.resize(imposedStressAtGaussPoints[0].size()) ;
            ret = 0 ;
...
void GeneralizedIterativeMaxwell::syncNumberOfGaussPoints(ElementState & state)
{
...
StandardLinearSolid::StandardLinearSolid(const Matrix & riga, const Matrix & rigb, const Matrix & e ) : LinearForm(riga, false, false, riga.numRows()/3+1), eta1(e), etaeq(e), decay(e), rig1(rigb), rig0(riga)
{
    Matrix s(e.numRows(), e.numRows()) ;
    s = rig1 ;
    Composite::invertTensor(s) ;
