...
WeibullStiffnessWithVariableImposedDeformationAndFracture::WeibullStiffnessWithVariableImposedDeformationAndFracture(const Matrix & rig, Vector imposedDef, FractureCriterion * c) : LinearForm(rig, true, false, rig.numRows()/3+1) , imposed(imposedDef), dfunc(rig.numRows()-1),criterion(c)
{
	frac = false ;
	init = param[0][0] ;
	change  = false ;
...
{
	f = VirtualMachine().ieval(Gradient(p_i,true) * ( (param * damage) * imposed), gp, Jinv,v) ;
}
