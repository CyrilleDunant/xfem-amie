...
StiffnessWithKineticsImposedDeformation::StiffnessWithKineticsImposedDeformation(const Matrix & rig, Vector imposedDef, Function k) : StiffnessWithImposedStrain(rig, imposedDef), kinetics(k)
{
    v.push_back(XI);
    v.push_back(ETA);
    if(param.size() == 36)
...
{
    StiffnessWithKineticsImposedDeformation * copy = new StiffnessWithKineticsImposedDeformation(*this) ;
...
void StiffnessWithKineticsImposedDeformation::step(double timestep, ElementState & currentState, double maxscore)
{
