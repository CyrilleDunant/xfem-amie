...
InCompleteCholesky::InCompleteCholesky(const CoordinateIndexedSparseMatrix &A_) : A(std::valarray<unsigned int>(1), std::valarray<unsigned int>(1))
{
	stable = true ;
	...
	A = CoordinateIndexedSparseMatrix(A_.row_size, A_.column_index) ;
...
// 			}
	...
	if (tries == 4)
	{
		...
		A = CoordinateIndexedSparseMatrix(rs, ci) ;
