...
#pragma omp parallel for
	for (int i = 0 ; i <end ; i++)
	{
		...
		for(size_t j = i*c.co.sm.stride ; j < i*c.co.sm.stride+c.co.sm.stride ; j++)
...
// 	std::cout << 2000000*c.co.sm.array.size()/delta << std::endl ;
	return ret ;
} ;
...
#pragma omp parallel for
		for (int i = 0 ; i < end; i++)
		{
			...
			for(size_t j = i*c.sm.stride ; j < i*c.sm.stride+c.sm.stride ; j++)
