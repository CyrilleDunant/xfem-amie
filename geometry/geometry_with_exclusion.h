...
public:
    ...
    virtual const PointArray& getBoundingPoints() const {
        return father->getBoundingPoints() ;
    }
    virtual PointArray& getBoundingPoints() {
        return father->getBoundingPoints() ;
    }
    virtual const Point& getBoundingPoint(size_t i) const {
        return father->getBoundingPoint(i) ;
    }
    virtual Point& getBoundingPoint(size_t i)  {
        return father->getBoundingPoint(i) ;
    }
    virtual void setBoundingPoints(const PointArray& p) {
        return father->setBoundingPoints(p) ;
    }
    virtual const Point& getPoint(size_t i) const  {
        return father->getPoint(i) ;
    }
    virtual Point& getPoint(size_t i)  {
        return father->getPoint(i) ;
    }
    virtual double getRadius() const {
        return father->getRadius() ;
    }
    virtual void sampleBoundingSurface(size_t n) { }
    virtual std::vector<Point> getSamplingBoundingPoints(size_t) const {
        std::vector<Point> ret ;
        return ret ;
    }
    virtual void sampleSurface(size_t) { }
    virtual std::vector<Point, std::allocator<Point> > getBoundingBox() const {
        return father->getBoundingBox() ;
    }
    virtual SpaceDimensionality spaceDimensions() const {
        return father->spaceDimensions() ;
    }
