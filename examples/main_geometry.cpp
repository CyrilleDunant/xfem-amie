...
/*        std::vector<Feature *> feats ;
        ...
        GeometryWithExclusion * sampleWithEllipse = new GeometryWithExclusion(dynamic_cast<Rectangle *>(&sample),dynamic_cast<Ellipse *>(ellipses[0])) ;
        feats = placement(sampleWithEllipse, feats, &nAgg, 0, 6400) ;
        
        for(int i = 0 ; i < feats.size() ; i++)
        {
        	if(ellipses[0]->intersects(feats[i]) || ellipses[0]->in(feats[i]->getCenter()))
        	{
        		std::cout << i << std::endl ;
        	}
        }
        ...
        for(size_t i = 0 ; i < ellipses.size() ; i++)
	{
                ellipses[i]->setBehaviour(new AggregateBehaviour()) ;
