...
int main(int argc, char *argv[])
{
	...
	for(size_t i = 0 ; i < 30 ; i++)
	{
		...
		f.step() ;
		out << f.getCurrentTime() << "\t" << f.getAverageField( REAL_STRESS_FIELD )[1]/1e6 << "\t" << f.getAverageField( MECHANICAL_STRAIN_FIELD )[1]*1e3 << "\t" << f.getAverageField( STRAIN_FIELD )[1]*1e3 << std::endl ;
	}
