...
int main(int argc, char *argv[])
{
	...
	for(size_t i = 0 ; i < 10 ; i++)
	{
		...
		f.step() ;
		out << f.getCurrentTime() << "\t" << f.getAverageField( REAL_STRESS_FIELD, 1. )[1]/1e6 << "\t" << f.getAverageField( TOTAL_STRAIN_FIELD, 1. )[1]*1e3 << "\t" << f.getAverageField( SCALAR_DAMAGE_FIELD, 1. )[0]*1e2 << std::endl ;
	}
