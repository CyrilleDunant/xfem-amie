...
{
	CommandLineParser parser("Builds a sample with a regular polygonal inclusion intersection masking a circular inclusion") ;
	...
	Inclusion inc2(&inc, 0.012, 0.004,0.005) ;
	...
	std::string name = "../examples/mesh/mesh_polygon_circle_mask_" ;
	if(renew)
		name += "base" ;
	else
		name += "current" ;
