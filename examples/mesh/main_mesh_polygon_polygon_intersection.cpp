...
{
	CommandLineParser parser("Builds a sample with two intersecting regular polygonal inclusions") ;
	...
	for(size_t i = 0 ; i < vertex.size() ; i++)
		vertex[i] = new Point( 0.014*cos(0.1+2.*M_PI*i/vertex.size()), 0.012*sin(0.1+2.*M_PI*i/vertex.size()) ) ;
	PolygonalSample inc(&rect, vertex) ;
	inc.setBehaviour(new Stiffness(20e9, 0.2) ) ;

	std::valarray<Point *> vertex2(5) ;
	for(size_t i = 0 ; i < vertex2.size() ; i++)
		vertex2[i] = new Point( -0.006+0.01*cos(0.1+2.*M_PI*i/vertex2.size()), 0+0.018*sin(0.1+2.*M_PI*i/vertex2.size()) ) ;
	PolygonalSample inc2(&inc, vertex2) ;
	...
	std::string name = "../examples/mesh/mesh_polygon_polygon_intersection_" ;
	if(renew)
		name += "base" ;
	else
		name += "current" ;
