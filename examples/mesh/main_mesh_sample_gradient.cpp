...
{
	CommandLineParser parser("Builds a sample without inclusions and a non-uniform mesh") ;
	...
	f.setSurfaceSamplingFactor(1.2) ;
	f.setSampler( &rect, new GradientSampler( Point(-0.02,0), Point(0.02,0), 0.5, 1.5) ) ;
	
	parser.setFeatureTree(&f) ;
	
	f.step() ;

	std::string name = dir+"/mesh_sample_gradient_" ;
	if(renew)
		name += "base" ;
	else
		name += "current" ;
