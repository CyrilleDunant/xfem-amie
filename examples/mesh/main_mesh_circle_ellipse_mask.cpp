...
{
	CommandLineParser parser("Builds a sample with a circular inclusion masking an ellipsoidal inclusion") ;
	...
	Inclusion inc(0.015,0,0) ;
	inc.setBehaviour(new Stiffness(20e9, 0.2) ) ;

	EllipsoidalInclusion inc2(Point(-0.005,0.005),Point(0.01,0.01),Point(0.01*0.7,-0.01*0.7)) ;
	...
	std::string name = "../examples/mesh/mesh_circle_ellipse_mask_" ;
	if(renew)
		name += "base" ;
	else
		name += "current" ;
