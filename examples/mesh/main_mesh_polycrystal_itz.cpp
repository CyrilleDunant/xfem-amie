...
{
	CommandLineParser parser("Builds a sample with a polycrystal microstructure with an interdistance of 0.001 between the grains") ;
	...
	FeatureTree f(&rect) ;
	PSDGenerator::get2DVoronoiPolygons( &f, grains, 0, 0.003, 0.01, 32, true, 0.001) ;
	...
	std::string name = "../examples/mesh/mesh_polycrystal_itz_" ;
	if(renew)
		name += "base" ;
	else
		name += "current" ;
