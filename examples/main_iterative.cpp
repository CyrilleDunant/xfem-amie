...
double r0 = 0.001 ;
double r1 = std::pow(1./0.63,0.3333333)*r0 ;
...
void step()
{
  ...
  int maxtries = 250 ;
	int tries = 0 ;
	featureTree->setMaxIterationsPerStep(250) ;
	...
	for(size_t i = 0 ; i < nsteps ; i++)
	{
		...
		double hom_area = 0 ;
		double avg_e_xx_hom = 0 ;
		double avg_e_xy_hom = 0 ;
		double avg_e_yy_hom = 0 ;
		double avg_s_xx_hom = 0 ;
		double avg_s_xy_hom = 0 ;
		double avg_s_yy_hom = 0 ;
...
// 						ex_count++ ;
				...
				if(triangles[k]->getCenter().norm() < r1)
				{
					hom_area += ar ;
					for(size_t l = 0 ; l < npoints ;l++)
					{
						avg_e_xx_hom += (epsilon11[k*npoints+l]/npoints)*ar;
						avg_e_yy_hom += (epsilon22[k*npoints+l]/npoints)*ar;
						avg_e_xy_hom += (epsilon12[k*npoints+l]/npoints)*ar;
						avg_s_xx_hom += (sigma11[k*npoints+l]/npoints)*ar;
						avg_s_yy_hom += (sigma22[k*npoints+l]/npoints)*ar;
						avg_s_xy_hom += (sigma12[k*npoints+l]/npoints)*ar;
					}
				}
   		...
   		if (tries < maxtries && featureTree->solverConverged())
		{
			...
			Matrix hom_m(3,3) ;
			hom_m[2][2] = avg_s_xy_hom / avg_e_xy_hom ;
			double E_2 = (avg_s_yy_hom - avg_s_xx_hom*avg_e_yy_hom/avg_e_xx_hom)/(avg_e_xx_hom-avg_e_yy_hom*avg_e_yy_hom/avg_e_xx_hom) ;
			double E_1 = (avg_s_xx_hom - E_2 * avg_e_yy_hom)/avg_e_xx_hom ;
			hom_m[0][0] = E_1 ;
			hom_m[1][1] = E_1 ;
			hom_m[0][1] = E_2 ;
			hom_m[0][1] = E_2 ;
			...
			Vector hom_a(3) ;
			hom_a[0] = avg_e_xx_hom ;
			hom_a[1] = avg_e_yy_hom ;
			hom_a[2] = avg_e_xy_hom ;
			
			hom_m.print() ;
			
			sample.setBehaviour(new StiffnessWithImposedDeformation(hom_m,hom_a)) ;
...
int main(int argc, char *argv[])
{
	...
	featureTree = &F ;
 	...
 	inclusions.push_back(new Inclusion(r0, 0., 0.)) ;
        inclusions[0]->setBehaviour(new AggregateBehaviour()) ;
	
	Inclusion * matrix = new Inclusion(r1,0.,0.) ;
	matrix->setBehaviour(new PasteBehaviour()) ;
	F.addFeature(&sample, matrix) ;
	
	Material mat_m(matrix->getBehaviour()->param) ;
	mat_m.addProperties(P_VOLUME_FRACTION,0.37) ;

	Material inc_m(inclusions[0]->getBehaviour()->param) ;
	inc_m.addProperties(P_VOLUME_FRACTION,0.63) ;
	
	Material mat_s ;
	mat_s.addPhase(mat_m) ;
	mat_s.addPhase(inc_m) ;
	mat_s.homogenize(ELASTICITY_MORI_TANAKA) ;
	
	double K = mat_s.valProperties(P_BULK_MODULUS) ;
	double G = mat_s.valProperties(P_SHEAR_MODULUS) ;

	sample.setBehaviour(new Stiffness(Material::cauchyGreen(std::make_pair(K,G), false, SPACE_TWO_DIMENSIONAL))) ;

	std::cout << r1 << std::endl ;

        for(size_t i = 0 ; i < inclusions.size() ; i++)
	{
		inclusions[i]->setFather(matrix) ;
                F.addFeature(matrix,inclusions[i]) ;
                placed_area += inclusions[i]->area() ;
        }
        

        zones = generateExpansiveZonesHomogeneously(atof(argv[2]), inclusions, F) ;
