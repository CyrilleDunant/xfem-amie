...
//		writer.getField(TWFT_VON_MISES) ;
		...
		{
			double delta_r = sqrt(aggregateArea*.01/((double)zones.size()*M_PI))/(double)nstepstot ;
			...
			for(size_t z = 0 ; z < zones.size() ; z++)
			{
				...
				{
					if(current_area/zones[z-1].areaOfInclusion() > 0.01)
					{
						stopped_reaction++ ;
...
std::vector<Zone> generateExpansiveZonesHomogeneously(int n, int max, std::vector<Inclusion * > & incs , FeatureTree & F)
{
	...
	for(size_t i = 0 ; i < zonesToPlace.size() ; i++)
	{
		...
		{
			Circle circle(incs[j]->getRadius() - radius*250, incs[j]->getCenter()) ;
...
// 	return 0 ;
	...
	F.addFeature(&sample, new ITZFeature( &sample, inclusions, 0.0015, 0.3333333, 1.)) ;
