...
#include "../physics/stress_defined_stiffness.h" 
...
int main(int argc, char *argv[])
{
  ...
  // Material behaviour of the matrix
	...
	for(size_t k = 0 ; k < triangles.size() ; k++)
	{
		if(triangles[k]->getBehaviour()->type == NON_LINEAR)
		{
			static_cast<StressDefinedStiffness *>(triangles[k]->getBehaviour())->setParent(triangles[k]) ;
		}
	}
