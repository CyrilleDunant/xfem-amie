...
\verb+..list_of_time_steps = ../examples/data/composite/time_steps.txt # name of the file containing the data+

\paragraph{} The series of instants must be strictly increasing; no time step must be equal to 0 (or lower). The first instant must be equal to 0.

\paragraph{} The file name must not contain a comma (\verb+,+), otherwise the second method is used.


\subsection{Defining a list of time steps}

Instead of calling a file, it is possible to directly define the list of instants in the input file:

\noindent \verb+.stepping+\\
\verb+..list_of_time_steps = 0,1,3 # list of instants separated by commas+

\paragraph{} The series of instants must be strictly increasing; no time step must be equal to 0 (or lower). The first instant must be equal to 0. The instants must be separated by commas (\verb+,+).
...
\paragraph{}Also, it is possible to import a file containing the values of the boundary condition at different instants:\\
...
\paragraph{}Instead of a file, it is also possible to directly input the values of the boundary condition in the initiation file:\\

\noindent \verb+.boundary_condition+\\
\verb+..condition = SET_STRESS_ETA+\\
\verb+..position = TOP+\\
\verb+..time_evolution+\\
\verb+...instants = 0,1,2 # list of instants separated by commas+\\
\verb+...values = 0,1e6,1e6 # list of values separated by commas+
...
\verb+ISOTROPIC_INCREMENTAL_\textit{LINEAR_}DAMAGE+ is a similar algorithm, except that it will apply a fixed amount of damage at each iteration. The amount can be fixed using the \verb+damage_increment+ variable:\\
...
\paragraph{} \verb+LINEAR_SOFTENING_MAXIMUM_TENSILE_STRAIN+ defines a material with a linear descending branch in the strain-stress diagram. \verb+limit_tensile_strain+ and \verb+limit_tensile_stress+ indicate the position of the peak, while \verb+maximum_tensile_strain+ indicates the ultimate strain at failure.\\
...
\begin{itemize}
	\item Import strain-stress curves from a text file
	\item Declare the strain-stress curve
...
\subsubsection{Import strain-stress curves}
...
\subsubsection{Declare a stress-strain curve}

\noindent \verb+.behaviour+\\
\verb+..type = LOGARITHMIC_CREEP+\\
\verb+..parameters+\\
\verb+...strain_stress_curve+\\
\verb+....tension+\\
\verb+.....strain = 0.0001,0.0002 # values of the strain separated by commas (must be positive)+\\
\verb+.....stress = 1e6,0 # values of the stress separated by commas (must be positive)+\\
\verb+....compression+\\
\verb+.....strain = -0.0001,-0.0002 # values of the strain separated by commas (must be negative)+\\
\verb+.....stress = -1e6,0 # values of the stress separated by commas (must be negative)+\\
\verb+...material_characteristic_radius = # value+\\
\verb+...strain_renormalization_factor = # optional, default value = 1e4+\\
\verb+...stress_renormalization_factor = # optional, default value = 1e-6+\\
\verb+...damage_increment = # value+\\
...
\paragraph{} The behaviour can be defined in tension, compression, or both. If \verb+tension+ (alternatively \verb+compression+) is not defined, then the material will not fail in tension (or in compression). If none of them are defined, then \textsc{amie} will try to use the other method described below.
...
\verb+...input_parameter = $input+\\
\verb+...output_parameter = $output+\\
\verb+...input_values = 0,1,2 # values of the input parameter separated by commas+\\
\verb+...output_values = 0,1,2 # values of the output parameter separated by commas+
