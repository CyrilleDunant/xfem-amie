...
bool PolygonalSample::inBoundary(const Point &p, double d) const
{
	...
	d_ = std::max( d, 0.3333*getPerimeter()/getBoundingPoints().size() ) ;
...
//    Point proj(p) ;
  //  project(&proj) ;
    return (squareDist3D(proj, p) < d_*d_) || in(p);
}

bool PolygonalSample::onBoundary(const Point &p, double d) const
{
    double d_ = d ;
    if( getBoundingPoints().size() > 0)
	d_ = std::max( d, 0.3333*getPerimeter()/getBoundingPoints().size() ) ;
