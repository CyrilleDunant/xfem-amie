...
namespace Amie
{
    ...
    FractureCriterion * Object::getFractureCriterion(std::string type, std::map<std::string, double> & values, std::map<std::string, std::string> & strings)
    {
        ...
        if( type == "SpaceTimeNonLocalLinearSofteningMaximumStrain" ) { return new SpaceTimeNonLocalLinearSofteningMaximumStrain(values["tensile_strain"], values["tensile_stress"], values["tensile_ultimate_strain"]) ; }
        if( type == "SpaceTimeNonLocalMaximumStress" ) { return new SpaceTimeNonLocalMaximumStress(values["tensile_stress"]) ; }
    ...
    void Object::resetFractureCriterion(FractureCriterion * target, std::map<std::string, double> & values, std::map<std::string, std::string> & strings)
    {
        ...
        // parsed from header file: ../physics/fracturecriteria/maxstrain.h
        if(dynamic_cast<SpaceTimeNonLocalLinearSofteningMaximumStrain *>(target) != nullptr) { dynamic_cast<SpaceTimeNonLocalLinearSofteningMaximumStrain *>(target)->reset(values["tensile_strain"], values["tensile_stress"], values["tensile_ultimate_strain"]) ; }
