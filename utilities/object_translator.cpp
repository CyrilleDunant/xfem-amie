...
namespace Amie
{
    ...
    FractureCriterion * Object::getFractureCriterion(std::string type, std::map<std::string, std::string> & strings, std::map<std::string, double> & values)
    {
        ...
        { 
            if( strings.find("frame") == strings.end() ) { strings["frame"] = "FRAME_CARTESIAN" ; } ; 
    ...
    PostProcessor * Object::getPostProcessor(std::string type, std::map<std::string, std::string> & strings, std::map<std::string, double> & values)
    {
        // parsed from header file: ../utilities/postprocessor.h
        if( type == "DoNothing" ) { return new DoNothingPostProcessor() ; }
        if( type == "AverageField" )
        { 
            if( values.find("index") == values.end() ) { values["index"] = -1 ; } ; 
            if( values.find("instant") == values.end() ) { values["instant"] = 1 ; } ; 
            return new AverageFieldPostProcessor(strings["field"], values["index"], values["instant"]) ;
        }
        if( type == "MinimumField" )
        { 
            if( values.find("index") == values.end() ) { values["index"] = -1 ; } ; 
            if( values.find("instant") == values.end() ) { values["instant"] = 1 ; } ; 
            return new MinimumFieldPostProcessor(strings["field"], values["index"], values["instant"]) ;
        }
        if( type == "MaximumField" )
        { 
            if( values.find("index") == values.end() ) { values["index"] = -1 ; } ; 
            if( values.find("instant") == values.end() ) { values["instant"] = 1 ; } ; 
            return new MaximumFieldPostProcessor(strings["field"], values["index"], values["instant"]) ;
        }
        if( type == "LocalField" )
        { 
            if( values.find("instant") == values.end() ) { values["instant"] = 1 ; } ; 
        ...
        { 
            if( values.find("instant") == values.end() ) { values["instant"] = 1 ; } ; 
            return new LinearStrainGaugePostProcessor(values["x1"], values["y1"], values["x2"], values["y2"], values["instant"]) ;
        }
        if( type == "MacroscopicStrain" )
        { 
            if( values.find("instant") == values.end() ) { values["instant"] = 1 ; } ; 
            return new MacroscopicStrainPostProcessor(values["x"], values["y"], values["instant"]) ;
        }
        if( type == "MaximumMacroscopicStrain" )
        { 
            if( values.find("divisions") == values.end() ) { values["divisions"] = 10 ; } ; 
            if( values.find("instant") == values.end() ) { values["instant"] = 1 ; } ; 
            return new MaximumMacroscopicStrainPostProcessor(values["divisions"], values["instant"]) ;
        }
   
        return nullptr ;
    }

    bool Object::isPostProcessor(std::string type)
    {
        // parsed from header file: ../utilities/postprocessor.h
        if( type == "DoNothing" ) { return true ; }
        if( type == "AverageField" ) { return true ; }
    ...
    void Object::resetPostProcessor(PostProcessor * target)
    {
        // parsed from header file: ../utilities/postprocessor.h
