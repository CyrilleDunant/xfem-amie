...
// 		valUnderCursor = prop * ( limits[currentSet].first - ( limits[currentSet].first - limits[currentSet].second ) * ( 1. - fracup / 10000. ) ) + ( 1. - prop ) * ( limits[currentSet].second + ( limits[currentSet].first - limits[currentSet].second ) * fracdown / 10000. ) ;
		...
		double downcut =  ( limits[currentSet].first - limits[currentSet].second ) * ( fracdown / 1000. ) ;
		double upcut =  ( limits[currentSet].second - limits[currentSet].first ) * ( fracup / 1000.-1. ) ;
...
void TriangleGLDrawer::paintGL()
{
	...
	if( !limits.empty() )
	{
	  COLOUR col ;
		...
		for( double i = ( double )fracup / 1000. ; i > ( double )fracdown / 1000. ; i -= 0.001 )
		{
			double where = ( 1. - ( ( double )fracup / 1000. - i ) / ( ( double )fracup / 1000. - ( double )fracdown / 1000. ) ) ;
// 			HSVtoRGB( &r, &g, &b, 180., 0., ( 1. - where ) ) ;
// 			HSVtoRGB( &r, &g, &b, 180.*( 1. - where ), 1., 1. ) ;
			col = GetColour(where) ;
			if(std::abs(limits[currentSet].second) < std::abs(limits[currentSet].first))
				col =  GetColour(1. - where) ;
			...
			glColor4ub( 255*col.r, 255*col.g, 255*col.b, 255 ) ;
...
// 	if(std::abs(limits[currentSet].second) <= std::abs(limits[currentSet].first))
        ...
        {
            double v = (i) * ( limits[currentSet].first - ( limits[currentSet].first - limits[currentSet].second ) * ( 1. - fracup / 1000. ) ) + ( 1. - i ) * ( limits[currentSet].second + ( limits[currentSet].first - limits[currentSet].second ) * fracdown / 1000. ) ;
            QString num = QString::number( v, 'g', 2) ;
...
/*        else
	for( double i = 1. ; i > 0 ; i -= 0.1 )
        {
            double v = (1.-i) * ( limits[currentSet].first - ( limits[currentSet].first - limits[currentSet].second ) * ( 1. - fracup / 1000. ) ) +  i  * ( limits[currentSet].second + ( limits[currentSet].first - limits[currentSet].second ) * fracdown / 1000. ) ;
...
//    }
//    
//    c.r *= 0.2+0.8*log(v*(M_E-1.)+1.) ;
//    c.g *= 0.2+0.8*log(v*(M_E-1.)+1.) ;
//    c.b *= 0.2+0.8*log(v*(M_E-1.)+1.) ;
//    
//    return(c);
// } ;
...
//   v= log(v*(M_E-1.)+1.) ;
  ...
  double _viridis_data[] = {0.267004, 0.004874, 0.329415,
   ...
   double t = sqrt(c.r*c.r+c.g*c.g+c.b*c.b) ;
   c.r *= sqrt(1.375)/t ;
   c.g *= sqrt(1.375)/t ;
   c.b *= sqrt(1.375)/t ;
	...
	for( size_t i = 0 ; i < numberOfTriangles ; i++ )
	{
		for( size_t j = 0 ; j < numberOfPointsPerTriangle ; j++ )
		{
			...
			if( ( *valuesAtPoint )[2 * j + 1][i] + ( *valuesAtPoint )[( 3 ) * numberOfPointsPerTriangle + j][i]> max_y )
				max_y = ( *valuesAtPoint )[2 * j + 1][i] + ( *valuesAtPoint )[( 3 ) * numberOfPointsPerTriangle + j][i];
			...
			if( ( *valuesAtPoint )[2 * j + 1][i] + ( *valuesAtPoint )[( 3 ) * numberOfPointsPerTriangle + j][i]< min_y )
				min_y = ( *valuesAtPoint )[2 * j + 1][i] + ( *valuesAtPoint )[( 3 ) * numberOfPointsPerTriangle + j][i];
		}
	}
